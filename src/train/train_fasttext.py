...
from src.model import FastText
...
def train(config, judger, config_proto):
    assert config.current_model == 'fasttext'
    ...
    with tf.variable_scope('model', reuse=None):
        train_model = FastText(
            accu_num=config.accu_num, max_seq_len=config.sentence_len, fc_size=config.fc_size_s,
