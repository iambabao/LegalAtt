...
def inference(sess, model, batch_iter, out_file, verbose=True):
    ...
    for i, batch in enumerate(batch_iter):
        ...
        fact, fact_seq_len, fact_doc_len, _ = list(zip(*batch))
...
def run_epoch(sess, model, batch_iter, verbose=True):
    ...
    for batch in batch_iter:
        fact, fact_seq_len, fact_doc_len, accu = list(zip(*batch))
...
def read_data(data_file, word_2_id, accu_2_id, max_seq_len, max_doc_len):
    ...
    return fact, fact_seq_len, fact_doc_len, accu
...
def train(judger, config_proto):
    ...
    train_data = read_data(config.TRAIN_DATA, word_2_id, accu_2_id, config.SEQUENCE_LEN, config.DOCUMENT_LEN)
    valid_data = read_data(config.VALID_DATA, word_2_id, accu_2_id, config.SEQUENCE_LEN, config.DOCUMENT_LEN)
